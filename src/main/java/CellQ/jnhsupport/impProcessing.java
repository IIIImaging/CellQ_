...
/**
 * These tools are free software; you can redistribute it and/or modify it under
...
public class impProcessing {
	...
	public static double getInterpolatedIntensity2D(ImagePlus imp, double x, double y, int z) {
		...
		intensity += imp.getStack().getVoxel((int) x, (int) y, z) * leftComponent * topComponent; // top
																									// left
																									// pixel
		intensity += imp.getStack().getVoxel((int) x + 1, (int) y, z) * (1.0 - leftComponent) * topComponent; // top
																												// right
																												// pixel
		intensity += imp.getStack().getVoxel((int) x, (int) y + 1, z) * leftComponent * (1.0 - topComponent); // bottom
																												// left
																												// pixel
		intensity += imp.getStack().getVoxel((int) x + 1, (int) y + 1, z) * (1.0 - leftComponent)
				* (1.0 - topComponent); // bottom right pixel
		return intensity;
		// IJ.log("Comp " + leftComponent + " " + topComponent);
		// IJ.log("sum " + (leftComponent * topComponent+(1.0-leftComponent) *
		// topComponent+leftComponent * (1.0-topComponent)+(1.0-leftComponent) *
		// (1.0-topComponent)));
		...
		for (int x = 0; x < imp.getWidth(); x++) {
			for (int y = 0; y < imp.getHeight(); y++) {
				for (int z = 0; z < imp.getStackSize(); z++) {
					...
					if ((includeZero || imp.getStack().getVoxel(x, y, z) != 0.0)
							&& imp.getStack().getVoxel(x, y, z) < min) {
						min = imp.getStack().getVoxel(x, y, z);
					}
				}
			}
	...
	public static ImagePlus getSingleImageFromStack(ImagePlus imp, int i) {
		/**
		 * Returns an independent ImagePlus of the selected stack image imp:
		 * image where single timepoint shall be derived from i: stack image 0
		 * <= i < # stack images)
		 */
	...
	/**
	 * @return an independent ImagePlus of the sharpest image in the ImagePlus
	 *         The sharpest image is defined as the image with the lowest
	 *         Standard Deviation
	 * @param imp:
	 *            stack-image where the sharpest image shall be found
	 */
	...
	public static ImagePlus getSharpestPlaneForSelection(ImagePlus imp, Roi roi) {
		/**
		 * Returns an independent ImagePlus of the sharpest image in the
		 * ImagePlus The sharpest image is defined as the image with the lowest
		 * Standard Deviation imp: stack-image where the sharpest image shall be
		 * found roi: the region of interest in which the SD is calculated
		 */
		double[] SD = new double[imp.getStackSize()];
		double maximum = 0.0;
		int maximumPos = -1;
	...
	public static ImagePlus maxIntensityProjectionOfSlices(ImagePlus imp) {
		ImagePlus impMax = IJ.createHyperStack("max s", imp.getWidth(), imp.getHeight(), imp.getNChannels(), 1,
				imp.getNFrames(), imp.getBitDepth());
				...
				for (int c = 0; c < imp.getNChannels(); c++) {
					for (int t = 0; t < imp.getNFrames(); t++) {
						for (int s = 0; s < imp.getNSlices(); s++) {
							if (imp.getStack().getVoxel(x, y, imp.getStackIndex(c + 1, s + 1, t + 1) - 1) > impMax
									.getStack().getVoxel(x, y, impMax.getStackIndex(c + 1, 1, t + 1) - 1)) {
	...
	public static ImagePlus maxIntensityProjectionOfTimesteps(ImagePlus imp) {
		ImagePlus impMax = IJ.createHyperStack("max t", imp.getWidth(), imp.getHeight(), imp.getNChannels(),
				imp.getNSlices(), 1, imp.getBitDepth());
			...
			for (int y = 0; y < imp.getHeight(); y++) {
				for (int c = 0; c < imp.getNChannels(); c++) {
					for (int s = 0; s < imp.getNSlices(); s++) {
						for (int t = 0; t < imp.getNFrames(); t++) {
							if (imp.getStack().getVoxel(x, y, imp.getStackIndex(c + 1, s + 1, t + 1) - 1) > impMax
									.getStack().getVoxel(x, y, impMax.getStackIndex(c + 1, s + 1, 1) - 1)) {
	...
	public static ImagePlus splitToStack(ImagePlus imp) {
		...
		ImagePlus impNew = IJ.createHyperStack("hyperstack image v1", newWidth, newHeight, channels, 4, frames,
				imp.getBitDepth());
		...
		if (frames > 1 && slices == 1) {
				...
				for (int t = 0; t < frames; t++) {
					for (int x = 0; x < newWidth; x++) {
						for (int y = 0; y < newHeight; y++) {
							...
							impNew.getStack().setVoxel(x, y, ns,
									iS.getVoxel(x + substractWidth, y + substractHeight, os));
			...
			impNew = IJ.createHyperStack("hyperstack image v2", newWidth, newHeight, channels, 4, slices,
					imp.getBitDepth());
			for (int c = 0; c < channels; c++) {
	...
	public static int getSharpestPlanePosition(ImagePlus imp, Roi roi) {
		/**
		 * @returns the index of the slice image which has the highest Standard
		 *          Deviation 0 < z < 1 imp: stack-image where the sharpest
		 *          image shall be found roi: the region of interest in which
		 *          the SD is calculated
		 */
		double[] SD = new double[imp.getStackSize()];
		double maximum = 0.0;
		int maximumPos = -1;
