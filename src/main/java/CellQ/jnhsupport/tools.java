...
public class tools {
	...
	public static void userCheckImage(ImagePlus imp) {
		imp.show();
		imp.updateAndDraw();
		new WaitForUserDialog("check!").show();
		imp.hide();
	...
	public static double getMedian(double[] values) {
		...
		if (medians.length % 2 == 0) {
			return (medians[(int) ((double) (medians.length) / 2.0) - 1]
	...
	/**
	 * @returns an int array. int [0] contains the array position of the highest
	 *          maximum, int [1] the position of the second highest maximum A
	 *          maximum is defined as an array position where the neighboring
	 *          position contain values below the position
	 * @param values:
	 *            the array to be investigated
	 * @param firstIndex
	 *            and @param lastIndex specify the range of the array, which is
	 *            investigated
	 */
	public static int[] get2HighestMaximaIndicesWithinRange(double[] values, int firstIndex, int lastIndex) {
		...
		// find first Maximum
		for (int x = firstIndex; x <= lastIndex && x < values.length; x++) {
			if (x > 0 && x < (values.length - 1) && values[x - 1] < values[x] && values[x + 1] < values[x]
					&& values[x] > maxima[0]) {
				maxima[0] = values[x];
				index[0] = x;
			}
	 ...
	 * @deprecated
	 * @returns an int array. int [0] contains the array position of the highest
	 *          maximum, int [1] the position of the second highest maximum to
	 *          the right of the highest maximum A maximum is defined as an
	 *          array position where the neighboring position contain values
	 *          below the position
	 * @param values:
	 *            the array to be investigated
	 * @param firstIndex
	 ...
	 *            investigated
	 */
	public static int[] get2FrequencyMaximaFromFFT(double[] values, int firstIndex, int lastIndex) {
		double[] maxima = { Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY };
		int[] index = { -1, -1 };
		...
		// find Maximum
		for (int x = index[0] + 1; x <= lastIndex && x < values.length; x++) {
			if (x > 0 && x < (values.length - 1) && values[x - 1] < values[x] && values[x + 1] < values[x]
					&& x != index[0] && values[x] > maxima[1]) {
				maxima[1] = values[x];
				index[1] = x;
	...
	public static double getCenterOfMassOfRange(double[] values, int start, int end) {
		double com = 0.0, sum = 0.0;
		for (int x = start; x <= end && x < values.length; x++) {
			com += (double) x * values[x];
			sum += values[x];
		}
		return com / sum;
	}
	...
	public static double get2DAngle(double[] u, double[] v) {
		...
		if ((theta) > 1.0) {
			// IJ.log("Theta determination: skp/lp > 1.0! " + "skp: " + skp + "
			// lp: " + lp);
			theta = 1.0;
		}
		...
		if (Double.isNaN(theta)) {
			if (lp == 0.0)
				IJ.log("lp=0 u" + u[0] + "-" + u[1] + "-" + u[2] + " v" + v[0] + "-" + v[1] + "-" + v[2]);
			if (Double.isNaN(u[0]))
				IJ.log("u0 NaN");
			if (Double.isNaN(u[1]))
				IJ.log("u1 NaN");
	...
	public static double getAbsoluteAngle(double[] u, double[] v) {
		...
		if (Double.isNaN(theta)) {
			...
			if (Double.isNaN(u[1]))
				IJ.log("u1 NaN");
			if (u.length > 2 && Double.isNaN(u[2]))
				IJ.log("u2 NaN");
			if (Double.isNaN(v[0]))
				IJ.log("v0 NaN");
			if (Double.isNaN(v[1]))
				IJ.log("v1 NaN");
			if (v.length > 2 && Double.isNaN(v[2]))
				IJ.log("v2 NaN");
		}
