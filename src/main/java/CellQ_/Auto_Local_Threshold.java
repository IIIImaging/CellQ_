...
public class Auto_Local_Threshold implements PlugIn {
	...
	public void run(String arg) {
		...
		String[] methods = { "Try all", "Bernsen", "Contrast", "Mean", "Median", "MidGrey", "Niblack", "Otsu",
				"Phansalkar", "Sauvola" };
		...
		if (myMethod.equals("Try all")) {
			...
			if (stackSize > 1 && doIstack) {
				...
				for (int j = 1; j <= stackSize; j++) {
					...
					mm.makeMontage(imp3, 3, 3, 1.0, 1, (ml - 1), 1, 0, true); // 3
																				// columns
																				// and
																				// 3
																				// rows
			...
			if (stackSize > 1 && doIstack) { // whole stack
				...
				// Object[] result = exec(imp, myMethod, noWhite, noBlack,
	...
	void Contrast(ImagePlus imp, int radius, double par1, double par2, boolean doIwhite) {
		// G. Landini, 2013
		// Based on a simple contrast toggle. This procedure does not have
		// user-provided paramters other than the kernel radius
		// Sets the pixel value to either white or black depending on whether
		// its current value is closest to the local Max or Min respectively
		// The procedure is similar to Toggle Contrast Enhancement (see Soille,
		// Morphological Image Analysis (2004), p. 259
		...
		byte[] min = (byte[]) ipMin.getPixels();
		for (int i = 0; i < pixels.length; i++) {
			pixels[i] = ((Math.abs((int) (max[i] & 0xff - pixels[i] & 0xff)) <= Math
					.abs((int) (pixels[i] & 0xff - min[i] & 0xff)))) ? object : backg;
		}
		// imp.updateAndDraw();
		return;
	}
	...
	void MidGrey(ImagePlus imp, int radius, double par1, double par2, boolean doIwhite) {
		...
		for (int i = 0; i < pixels.length; i++) {
			pixels[i] = ((int) (pixels[i] & 0xff) > (int) (((max[i] & 0xff) + (min[i] & 0xff)) / 2) - c_value) ? object
					: backg;
		}
		// imp.updateAndDraw();
		return;
	...
	void Niblack(ImagePlus imp, int radius, double par1, double par2, boolean doIwhite) {
		...
		if (par2 != 0) {
			IJ.log("Niblack: changed c_value from :" + c_value + "  to:" + par2);// requested
																					// feature,
																					// not
																					// in
																					// original
		...
		for (int i = 0; i < pixels.length; i++)
			pixels[i] = ((int) (pixels[i] & 0xff) > (int) (mean[i] + k_value * Math.sqrt(var[i]) - c_value)) ? object
					: backg;
		// imp.updateAndDraw();
		return;
	}
	...
	void Otsu(ImagePlus imp, int radius, double par1, double par2, boolean doIwhite) {
		...
		for (int y = 0; y < h; y++) {
			...
			for (int x = 0; x < w; x++) {
				...
				// Calculate the BCV at each gray-level and find the threshold
				// that maximizes it
	...
	public static void Phansalkar(ImagePlus imp, int radius, double par1, double par2, boolean doIwhite) { // changed
																											// to
																											// static!
		// This is a modification of Sauvola's thresholding method to deal with
		// low contrast images.
		// Phansalskar N. et al. Adaptive local thresholding for detection of
		// nuclei in diversity stained
		// cytology images.International Conference on Communications and Signal
		// Processing (ICCSP), 2011,
		// 218 - 220.
		// In this method, the threshold t =
		// mean*(1+p*exp(-q*mean)+k*((stdev/r)-1))
		// Phansalkar recommends k = 0.25, r = 0.5, p = 2 and q = 10. In this
		// plugin, k and r are the
		...
		for (int i = 0; i < pixels.length; i++)
			pixels[i] = ((ori[i]) > (mean[i]
					* (1.0 + p_value * Math.exp(-q_value * mean[i]) + k_value * ((sd[i] / r_value) - 1.0)))) ? object
							: backg;
		// imp.updateAndDraw();
		return;
	}
	...
	void Sauvola(ImagePlus imp, int radius, double par1, double par2, boolean doIwhite) {
		...
		// Sauvola J. and Pietaksinen M. (2000) "Adaptive Document Image
		// Binarization"
		...
		for (int i = 0; i < pixels.length; i++)
			pixels[i] = ((int) (pixels[i] & 0xff) > (int) (mean[i]
					* (1.0 + k_value * ((Math.sqrt(var[i]) / r_value) - 1.0)))) ? object : backg;
		// imp.updateAndDraw();
		return;
	}

	private static ImagePlus duplicateImage(ImageProcessor iProcessor) { // changed
																			// to
																			// static!
